 ...
 * 每次取最小权值的邻接边，将该邻接边连接的顶点与原先顶点构成一棵树，接着寻找 下一个顶点，继续按照邻接边权值升序进行比较，取权值最小的构成树...
 * 
 * 该类用一个Edge类构成一个邻接边的信息，包括邻接边的起始顶点与结束顶点，权值。
 * 用类Edge创建对象，录入对象信息，按照对象的权值进行比较，符合条件的对象加入 到链表中，最终按照链表顺序输出最小生成树。
 */
public class KruskalTree {
	...
	/*
	 * Max:定义顶点数组的最大值 edge:链表edge，存储构造的Edge对象 target:链表trget,存储最终得到结果的Edge对象
	 * parent:存储顶点信息的数组 n:顶点数
	 */
	int Max = 100;
	...
	public void init() {
		...
		n = scan.nextInt();
		System.out
				.println("按照'A,B,C'的格式输入边与边的信息,ABC分别代表边的起始顶点，结束顶点，权值(输入-1 -1 -1结束输入):");
		while (true) {
			p = scan.nextInt();
			q = scan.nextInt();
			w = scan.nextDouble();
	...
	public static void main(String[] args) {
		...
		KruskalTree kr = new KruskalTree();
		kr.init();
		kr.kruskal();
	}
}
